...
\newcommand{\drawEightSize}{\emph{[pending]}}
...
\newcommand{\pvLossVIIIc}{\emph{[pending: 1.c4 line]}}
...
6 & loss & $+1$ & 100/0/0 & $-1$ & 56/0/44\\
6 & draw & $+1$ & 100/0/0 & $-1$ & 98/1/1\\
\bottomrule
\end{tabular}
\caption{Tablebase versus Stockfish, 100 games per side. Stockfish acts as a move
evaluator (never moving a king) and searches to a fixed depth of $12$ at every
move---a constant per-position budget---while the tablebase chooses a random
optimal move each turn. ``theory'' is the game value from that side; W/D/L are
from the tablebase's viewpoint. The tablebase never scores below theory: it wins
every game from a winning side, never loses from a drawing side, and loses only
from a theoretically-lost side. (Runs for $n=7,8$ use the same protocol driven by
the C\texttt{++} table; see the text.)}
\label{tab:stockfish}
\end{table}
...
\midrule
1 & 0 & --- & $<0.01$ & 0 & --- & $<0.01$ \\
2 & 5 & $<$1\,KB & $<0.01$ & 13 & $<$1\,KB & $<0.01$ \\
3 & 96 & $<$2\,KB & $<0.01$ & 176 & $<$3\,KB & $<0.01$ \\
4 & \num{1333} & 21\,KB & $<0.01$ & \num{5405} & 86\,KB & 0.01 \\
5 & \num{85303} & 1.4\,MB & 0.17 & \num{218930} & 3.5\,MB & 0.31 \\
6 & \num{1105031} & 18\,MB & 2.07 & \num{4557283} & 73\,MB & 6.44 \\
7 & \num{35511940} & 0.57\,GB & 72.9 & \num{146817313} & 2.3\,GB & 295.3 \\
8 & \nEightLossStates & 13.2\,GB & $2760^{\dagger}$ & \drawEightStates & \drawEightSize & \drawEightTime \\
\bottomrule
\end{tabular}
\caption{Cached states, table size, and single-run wall time on the Apple
M5~Pro. Each stored position occupies exactly 16 bytes, so ``size'' is
$16\times$ the state count. Rows $n\le7$ are single-threaded; $n=8$ ($\dagger$)
uses six threads. State counts are the positions stored at the subtree-storage
threshold in use ($\text{minsub}=16$ for $n\le7$, larger for $n=8$ so the tables
fit in memory), so they measure solve size rather than the full reachable set.}
\label{tab:scale}
\end{table}
