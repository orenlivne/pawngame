...
struct TT {
    u128* slots = nullptr;
    size_t mask = 0, cap = 0, count = 0;
    void init(int bits) { cap = (size_t)1 << bits; slots = (u128*)calloc(cap, sizeof(u128)); mask = cap - 1; if (!slots) { fprintf(stderr, "alloc failed for 2^%d slots\n", bits); exit(2); } }
    ...
    inline size_t probe(u128 p) const { size_t i = hsh(p) & mask; while (true) { u128 e = slots[i]; if (e == 0) return i; if ((e >> 4) == p) return i; i = (i + 1) & mask; } }
};
...
struct Solver {
    ...
    int solve(const Board& b, int alpha, int beta) {
        ...
        if (nodes - nb >= minsub) {
            ...
            u128 e2 = tt.slots[idx2];
            if (e2 == 0) {
                tt.slots[idx2] = (p << 4) | ((u128)flag << 2) | (u128)(best + 2);
                ++tt.count;
                if ((tt.count & ((1ULL << 25) - 1)) == 0)
                    fprintf(stderr, "  [progress] stored=%zu (%.1fGB) visits=%llu\n", tt.count, (double)tt.count * 16 / 1e9, nodes);
                if (tt.count * 10 > tt.cap * 9) { fprintf(stderr, "TT overflow (>90%% of 2^%zu). Increase --bits.\n", (size_t)__builtin_ctzll((unsigned long long)tt.cap)); exit(3); }
            } else {
                // Slot already holds this position: keep the more informative entry.
...
int main(int argc, char** argv) {
    ...
    double gb = (double)s.tt.count * 16.0 / 1e9;
    printf("n=%d rule=%s justify=%s sym=%d colorsym=%d race=%d minsub=%llu value=%d (%s) stored=%zu (%.1fGB) visits=%llu %.2fs (%.1fM/s)\n",
           n, loss ? "loss" : "draw", justify.c_str(), use_sym, g_colorsym, use_race, minsub,
           v, lab, s.tt.count, gb, s.nodes, dt, s.nodes / dt / 1e6);
    if (want_pv) {
