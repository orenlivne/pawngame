...
move. We solve the game exactly for $n=1,\dots,8$ pawns per side under two rulesets that differ only in the treatment of a player with
...
eight-pawn game is a first-player win; White wins for $n=6,7,8$ and for
...
independent engine and cross-validating every shortcut against unpruned search; it
is further stress-tested by a play-test in which the tablebase never loses to
Stockfish from a non-losing side. We also solve a variant in which a player may
pass, but not twice running: there the game is a first-player win under the stated
rule (for $n\le 8$) and a draw under chess stalemate (verified for $n\le 7$, and
conjectured beyond by the constancy of the value), showing the outcomes to be
governed by zugzwang. A section aimed at chess players interprets the
solution.
\end{abstract}
...
the flank, and White promotes it one move ahead of Black's counterplay because the
combination of moving first and the tempo gained by the break wins the race. The
four-pawn win is the clearest miniature (Figure~\ref{fig:break}): \nEightLossPVfour
...
four-pawn miniature enlarged: \nEightLossPVeight. There is no analogue of a piece
opening such as the Ruy Lopez here; the relevant reference point is the endgame
breakthrough technique---executed here starting from the first move.
...
(Table~\ref{tab:stockfish}); and whenever White wins, Black is lost from move
one, so the lines in Appendix~\ref{app:pv} are each one optimal continuation, not
a refuted blunder.
...
I am grateful to FIDE Master Carl Boor, who introduced me to the pawn game and whose
questions---on the pattern of the values, on what happens if a player may pass, and
on which pawn structures still break through under passive defense---prompted the
pass variant of Section~\ref{sec:pass} and the discussion of patterns.
